...
def init():
	...
	relation2id = {}
	f = open('./origin_data/relation2idWithoutNoEdge.txt', 'r')
	while True:
		content = f.readline()
		if content == '':
			break
	...
	train_sen_id = {}
	#需要加载random_index_list.data
	random_index_list = pickle.load(open('/Users/mac/Downloads/random_index_list.data','r'))
	...
	while True:
		...
		if content[2] not in relation2id:
			raise RuntimeError(content[2]+' not in relation2id dictionary!')
		else:
		...
		train_sen_id[tup][label_tag].append(random_index_list[idx_counter])
	...
	test_sen_id = {}
	f = open('./origin_data/testset_AR_RE_3Thousand_4gcn.txt','r')
	...
	while True:
		...
		test_sen_id[tup].append(random_index_list[idx_counter])
		idx_counter = idx_counter + 1;
	train_x = []
	train_y = []
	train_x_id = []
	...
	for i in train_sen:
		...
		for j in range(lenth):
			...
			train_x_id.append(train_sen_id[i][j])
	...
	np.save('./data/train_x_id.npy', train_x_id)
